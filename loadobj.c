...
static char *readline(FILE *f)
{
   ...
   while ((ch = fgetc(f)) != EOF)
   {
      ...
      if (k >= linelen)
      {
         ...
         if (!line)
            Fatal("Out of memory in readline\n");
      }
      //  End of Line
      if (CRLF(ch))
      {
...
static void readfloat(char *line, int n, float x[])
{
   for (int i = 0; i < n; i++)
   {
      ...
      if (!str)
         Fatal("Premature EOL reading %d floats\n", n);
      if (sscanf(str, "%f", x + i) != 1)
         Fatal("Error reading float %d\n", i);
   }
}
...
static void LoadMaterial(const char *file)
{
   ...
   while ((line = readline(f)))
   {
      ...
      if ((str = readstr(line, "newmtl")))
      {
         ...
         if (!mtl[k].name)
            Fatal("Cannot allocate %d for name\n", l + 1);
...
//  Set material
//
static void SetMaterial1(const char *name)
{
   ...
   while ((line = readline(f)))
   {
      ...
      else if (line[0] == 'f')
      {
         ...
         while ((str = getword(&line)))
         {
            ...
            {
               if (Kv < 0 || Kv > Nv / 3)
                  Fatal("Vertex %d out of range 1-%d\n", Kv, Nv / 3);
               if (Kn < 0 || Kn > Nn / 3)
                  Fatal("Normal %d out of range 1-%d\n", Kn, Nn / 3);
               if (Kt < 0 || Kt > Nt / 2)
                  Fatal("Texture %d out of range 1-%d\n", Kt, Nt / 2);
            }
